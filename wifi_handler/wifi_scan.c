...
static void wifi_scan_task(void *pvParameters)
{
    ...
    // Record our task handle for suspend/resume API
...
/* Suspend (pause) the scanning task */
void wifi_scan_suspend(void) {
    if (scan_task_handle) {
        vTaskSuspend(scan_task_handle);
        ESP_LOGI(TAG, "WiFi scan task suspended.");
    }
}

/* Resume (un-pause) the scanning task */
void wifi_scan_resume(void) {
    if (scan_task_handle) {
        vTaskResume(scan_task_handle);
        ESP_LOGI(TAG, "WiFi scan task resumed.");
    }
}
