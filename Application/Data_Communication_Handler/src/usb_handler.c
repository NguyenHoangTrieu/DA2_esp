...
/**
 * @brief JTAG handler task - receives data from USB Serial JTAG and sends to
 * config handler Checks for "CF" prefix to identify configuration commands
...
static void jtag_task(void *arg) {
  ...
  ESP_LOGI(TAG,
           "JTAG handler task started, listening for 'CF' prefix commands");
