 ...
 * converts it to its own clock via `bench_time_sync_from_peer_us()` and
 * computes total internal E2E as `(wan_now_us − converted_lan_rx_us)` —
 * a single subtraction, no sum-of-segments estimation. Zero = not measured.
 */
typedef struct __attribute__((packed)) {
 ...
 * on RX and `wan_t3_us` right before loading the response, then returns
 * tsync_response_t carrying both timestamps + echo of t1.
 */
typedef struct __attribute__((packed)) {
  uint16_t header;    // 0x4346 (CF)
