...
static void process_data_from_lan(const uint8_t *payload, uint16_t length) {
  ...
  // ===== Benchmark throughput sink (HANDLER_BENCH / "BNC") =====
  // BNC frames are generated by the LAN-side bench_throughput task purely to
  // saturate the SPI link.  Count bytes for the reporter and ACK immediately;
