...
#define _PARSE_FIELD_START(i, ptr, sep, sep_count)                             \
  ((i) == 0 ? (ptr) : sep[(i) - 1] + 1)
#define _PARSE_FIELD_END(i, end, sep, sep_count)                               \
  ((i) < (sep_count) ? sep[i] : (end))
#define _PARSE_FIELD_LEN(i, ptr, end, sep, sep_count)                          \
  ((int)(_PARSE_FIELD_END(i, end, sep, sep_count) -                            \
         _PARSE_FIELD_START(i, ptr, sep, sep_count)))

// External global config contexts (defined in wifi_connect.c, lte_connect.c,
// mqtt_handler.c)
extern wifi_config_context_t g_wifi_ctx;
extern lte_config_context_t g_lte_ctx;
extern mqtt_config_context_t g_mqtt_ctx;

// HTTP and CoAP config globals (used by config_handler +
// http_handler/coap_handler)
...
config_internet_type_t g_internet_fallback_type =
    CONFIG_INTERNET_WIFI; /* WiFi fallback */

static bool config_handler_running = false;
static TaskHandle_t config_handler_task_handle = NULL;

// pre define functions
static esp_err_t config_parse_wifi(const char *data, uint16_t len,
                                   wifi_config_data_t *cfg);
static esp_err_t config_parse_lte(const char *data, uint16_t len,
                                  lte_config_data_t *cfg);
static esp_err_t config_parse_mqtt(const char *data, uint16_t len,
                                   mqtt_config_data_t *cfg);
static esp_err_t config_parse_internet(const char *data, uint16_t len,
                                       config_internet_type_t *type);
static esp_err_t config_parse_server_type(const char *data, uint16_t len,
                                          config_server_type_t *type);
...
esp_err_t config_update_lte_safe(const lte_config_data_t *new_cfg) {
  ...
  if (xSemaphoreTake(g_config_context_mutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
    ...
    strncpy(g_lte_ctx.modem_name, new_cfg->modem_name,
            sizeof(g_lte_ctx.modem_name) - 1);
...
static esp_err_t config_parse_wifi(const char *data, uint16_t len,
  ...
  if (!data || !cfg || len < CONFIG_CMD_MIN_LEN) {
    ESP_LOGE(TAG,
             "WiFi config parse: invalid parameters (data=%p, cfg=%p, len=%u)",
             data, cfg, len);
    return ESP_FAIL;
  }
  ...
  // Parse format: "WF:SSID:PASSWORD:AUTH_MODE" or
  ...
  ESP_LOGI(
      TAG,
      "Parsed WiFi config - SSID: '%s', Pass: '%s', Username: '%s', Auth: %d",
 ...
 * Example: "LT:A7600C1:m-wap:user:pass:USB:true:30000:0:05:06"
...
static esp_err_t config_parse_lte(const char *data, uint16_t len,
  ...
  ESP_LOGI(TAG,
           "Parsed LTE config - Modem: '%s', APN: '%s', Usr: '%s', "
           "CommType: %d, AutoConn: %d, Timeout: %lu, MaxRetry: %lu, "
           "PwrPin: %u, RstPin: %u",
           cfg->modem_name, cfg->apn, cfg->username, cfg->comm_type,
           cfg->auto_reconnect, (unsigned long)cfg->reconnect_timeout_ms,
           (unsigned long)cfg->max_reconnect_attempts, cfg->pwr_pin,
           cfg->rst_pin);
  return ESP_OK;
}
 ...
 * Example:
 * "MQ:mqtt://demo.thingsboard.io:1883|myDeviceToken123|subTopic|pubTopic|attrTopic"
 * @param data Raw command data
 * @param len Command length
...
static esp_err_t config_parse_mqtt(const char *data, uint16_t len,
  ...
  ESP_LOGI(TAG,
           "Parsed MQTT config - URI: '%s', Token: '%s', Sub: '%s', Pub: '%s', "
           "Attr: '%s', keepalive=%us, timeout=%lums",
           cfg->broker_uri, cfg->device_token, cfg->subscribe_topic,
           cfg->publish_topic, cfg->attribute_topic, cfg->keepalive_s,
 ...
 * Format: "IN:TYPE" or "IN:TYPE:FALLBACK_ENABLE" or
 * "IN:TYPE:FALLBACK_ENABLE:FALLBACK_TYPE" Examples: "IN:WIFI"         — set
 * WiFi, keep fallback state "IN:LTE:1"        — set LTE, enable fallback
 * (auto→WIFI) "IN:LTE:0"        — set LTE, disable fallback "IN:LTE:1:WIFI"   —
 * set LTE, enable fallback, explicitly fallback to WIFI "IN:WIFI:1:LTE"   — set
 * WiFi, enable fallback, explicitly fallback to LTE
 *
...
static esp_err_t config_parse_internet(const char *data, uint16_t len,
  ...
  if (colon && colon + 1 < end) {
    ...
    if (fb_ch == '1') {
      ...
      ESP_LOGI(TAG, "Fallback ENABLED (type=%d, auto)",
  ...
  ESP_LOGI(TAG, "Parsed Internet type: %d, fallback: %d (type=%d)", *type,
           g_internet_fallback, g_internet_fallback_type);
  return ESP_OK;
}
...
static esp_err_t config_parse_http(const char *data, uint16_t len,
  ...
  ESP_LOGI(TAG, "Parsed HTTP config - URL: '%s', TLS: %d", cfg->server_url,
           cfg->use_tls);
  return ESP_OK;
}
...
static esp_err_t config_parse_coap(const char *data, uint16_t len,
  ...
  ptr = sep7 + 1;
  {
    ...
    if (poll_ms > 0)
      cfg->rpc_poll_interval_ms = poll_ms;
  }

done:
  ESP_LOGI(TAG, "Parsed CoAP config - Host: '%s', Resource: '%s', poll_ms=%lu",
           cfg->host, cfg->resource_path,
           (unsigned long)cfg->rpc_poll_interval_ms);
  return ESP_OK;
}

/**
 * @brief Config handler task - receives raw commands and routes to specific
 * queues
    ...
    if (xQueueReceive(g_config_handler_queue, &cmd, pdMS_TO_TICKS(100)) ==
      ...
      case CONFIG_TYPE_LTE: {
        ...
        if (config_parse_lte(cmd->raw_data, cmd->data_len, &lte_cfg) ==
          ...
          strncpy(g_lte_ctx.password, lte_cfg.password,
                  sizeof(g_lte_ctx.password) - 1);
          ...
          strncpy(g_lte_ctx.modem_name, lte_cfg.modem_name,
                  sizeof(g_lte_ctx.modem_name) - 1);
...
void config_handler_task_start(void) {
  ...
  if (!g_wifi_config_queue) {
    g_wifi_config_queue =
        xQueueCreate(CONFIG_QUEUE_SIZE, sizeof(wifi_config_data_t));
  }

  if (!g_mqtt_config_queue) {
    g_mqtt_config_queue =
        xQueueCreate(CONFIG_QUEUE_SIZE, sizeof(mqtt_config_data_t));
  }

  if (!g_lte_config_queue) {
    g_lte_config_queue =
        xQueueCreate(CONFIG_QUEUE_SIZE, sizeof(lte_config_data_t));
  }
    ...
    if (cfg_stack && cfg_tcb) {
      config_handler_task_handle =
          xTaskCreateStatic(config_handler_task, "config_handler", 4096, NULL,
                            5, cfg_stack, cfg_tcb);
      ESP_LOGI(TAG, "Config handler task created in internal RAM");
    } else {
      ESP_LOGE(TAG, "Failed to allocate config_handler task in internal RAM");
    }
  }
}
