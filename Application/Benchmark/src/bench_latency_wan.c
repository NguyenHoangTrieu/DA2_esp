...
static void reporter_task(void *arg)
{
    ...
    while (1) {
        ...
        uint32_t n, loss, fail, tmo;
        uint64_t mn, mx, sum;
        portENTER_CRITICAL(&s_mux);
        n = s_w.n; loss = s_w.loss; fail = s_w.send_fail; tmo = s_w.tx_timeout;
        mn = s_w.lat_min_us; mx = s_w.lat_max_us; sum = s_w.lat_sum_us;
        s_w.n = 0; s_w.loss = 0; s_w.send_fail = 0; s_w.tx_timeout = 0;
        s_w.lat_sum_us = 0;
        s_w.lat_min_us = UINT64_MAX; s_w.lat_max_us = 0;
        portEXIT_CRITICAL(&s_mux);

        if (n == 0) {
            ESP_LOGI(TAG, "WIN card=%s n=0 (idle)", card_label());
        } else {
            double mn_ms  = mn == UINT64_MAX ? 0.0 : (double)mn / 1000.0;
            double mx_ms  = (double)mx / 1000.0;
            double avg_ms = (double)(sum / n) / 1000.0;
            ESP_LOGI(TAG, "WIN card=%s n=%lu min=%.3f ms avg=%.3f ms max=%.3f ms loss=%lu send_fail=%lu tx_tmo=%lu",
                     card_label(),
                     (unsigned long)n, mn_ms, avg_ms, mx_ms,
                     (unsigned long)loss, (unsigned long)fail, (unsigned long)tmo);
        }
    }
}
...
void bench_latency_wan_on_frame(const uint8_t *payload, uint16_t len)
{
    ...
    if (xQueueSend(s_q, &item, 0) != pdTRUE) {
        ESP_LOGW(TAG, "lat queue full — dropping seq=%lu", (unsigned long)item.seq);
    }
}
