...
esp_err_t api_config_get_handler(void *arg) {
  ...
  cJSON_AddNumberToObject(jl, "reconnect_timeout_ms",
                          (double)lte.reconnect_timeout_ms);
  cJSON_AddNumberToObject(jl, "max_reconnect_attempts",
                          (double)lte.max_reconnect_attempts);
  /* Convert numeric stack enum index to TCA port-pin label ("04"–"17") */
  char ppin_str[4], rpin_str[4];
  snprintf(ppin_str, sizeof(ppin_str), "%d%d", lte.pwr_pin / 8,
           lte.pwr_pin % 8);
  snprintf(rpin_str, sizeof(rpin_str), "%d%d", lte.rst_pin / 8,
           lte.rst_pin % 8);
  ...
  cJSON_AddNumberToObject(jc, "ack_timeout_ms",
                          (double)g_coap_cfg.ack_timeout_ms);
  cJSON_AddNumberToObject(jc, "max_retransmit", g_coap_cfg.max_retransmit);
  cJSON_AddNumberToObject(jc, "rpc_poll_interval_ms",
                          (double)g_coap_cfg.rpc_poll_interval_ms);
  ...
  cJSON_AddStringToObject(jwan, "stack_wan_id",
                          (wan_stack_id && wan_stack_id[0]) ? wan_stack_id
                                                            : "100");
  ...
  char lan_s2[4] = "000";
  {
    ...
    if (lan_buf) {
      uint16_t lan_len = 0;
      esp_err_t lan_ret =
          mcu_lan_handler_request_config_async(lan_buf, &lan_len, 255, 2000);
...
static esp_err_t build_wifi_cmd(const cJSON *obj, config_result_waiter_t *waiter,
  ...
  if (uname[0] != '\0') {
    ...
    len = snprintf(wire, sizeof(wire), "WF:%s:%s:%s:%s", ssid->valuestring, pw,
                   uname, amode);
  } else {
...
static esp_err_t build_lte_cmd(const cJSON *obj, config_result_waiter_t *waiter,
  ...
  /* Use default APN (Vietnamobile m-wap) if not provided or empty */
  const char *apn_str =
      (apn && cJSON_IsString(apn) && apn->valuestring[0] != '\0')
          ? apn->valuestring
          : "m-wap";
  ...
  const char *comm = "USB";
  if (cJSON_IsNumber(ct) && ct->valueint == 1)
    comm = "UART";
...
static esp_err_t build_mqtt_cmd(const cJSON *obj,
  ...
  char wire[1024];
  int len =
      snprintf(wire, sizeof(wire), "MQ:%s|%s|%s|%s|%s|%u|%lu", uri->valuestring,
               tok->valuestring, cJSON_IsString(sub) ? sub->valuestring : "",
...
static esp_err_t build_http_cmd(const cJSON *obj, config_result_waiter_t *waiter,
  ...
  char wire[512];
  int len =
      snprintf(wire, sizeof(wire), "HP:%s|%s|%d|%d|%d|%lu", url->valuestring,
...
static esp_err_t build_coap_cmd(const cJSON *obj, config_result_waiter_t *waiter,
  ...
  char wire[512];
  int len = snprintf(
      wire, sizeof(wire), "CP:%s|%s|%s|%d|%d|%lu|%d|%lu", host->valuestring,
...
static esp_err_t build_wan_cmd(const cJSON *obj, config_result_waiter_t *waiter,
  ...
  if (strcmp(type_str, "WIFI") != 0 && strcmp(type_str, "LTE") != 0 &&
      strcmp(type_str, "ETHERNET") != 0 && strcmp(type_str, "NBIOT") != 0) {
    return ESP_ERR_INVALID_ARG;
  }
  ...
  if (cJSON_IsNumber(jfb) && jfb->valueint != 0 && cJSON_IsString(jfb_type)) {
    ...
    len = snprintf(wire, sizeof(wire), "IN:%s:1:%s", type_str,
                   jfb_type->valuestring);
  } else if (cJSON_IsNumber(jfb)) {
    len = snprintf(wire, sizeof(wire), "IN:%s:%d", type_str,
                   jfb->valueint ? 1 : 0);
  } else {
