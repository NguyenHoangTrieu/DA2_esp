...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base,
  ...
  case MQTT_EVENT_CONNECTED:
    ...
    esp_mqtt_client_subscribe(event->client, ATTRIBUTE_TOPIC, 1);
    esp_mqtt_client_subscribe(event->client, SUBSCRIBE_TOPIC, 1);
    esp_mqtt_client_publish(event->client, ATTRIBUTE_TOPIC,
  ...
  while (!mqtt_task_close) {
    if (g_mqtt_publish_queue) {
      if (xQueueReceive(g_mqtt_publish_queue, &incoming_data,
        ...
        int msg_id = esp_mqtt_client_publish(m_client, PUBLISH_TOPIC,
