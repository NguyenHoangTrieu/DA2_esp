...
esp_err_t ppp_server_trigger_lan_ota(void)
{
    ...
    // Check if PPP client is connected
    if (!s_ppp_client_connected) {
        ESP_LOGW(TAG, "PPP client not connected, waiting...");
        EventBits_t bits = xEventGroupWaitBits(
            s_ppp_event_group,
            PPP_CLIENT_CONNECTED_BIT,
            pdFALSE,
            pdFALSE,
            pdMS_TO_TICKS(10000)  // 10 second timeout
        );
        
        if (!(bits & PPP_CLIENT_CONNECTED_BIT)) {
            ESP_LOGE(TAG, "PPP client connection timeout");
    ...
    // For OTA trigger, implement via HTTP/MQTT over PPP connection
    ESP_LOGW(TAG, "OTA trigger should be implemented via HTTP/MQTT over PPP");
    ESP_LOGW(TAG, "Example: HTTP POST to http://192.168.11.2/api/ota/trigger");

    // Reset response tracking
    s_last_response_result = ESP_ERR_TIMEOUT;
    xSemaphoreTake(s_response_semaphore, 0);

    ESP_LOGI(TAG, "OTA trigger prepared");
    return ESP_OK;
}
...
esp_err_t ppp_server_wait_for_lan_response(TickType_t timeout)
{
    ...
    // Wait for semaphore signal (should be given by HTTP response handler)
