...
static void lte_config_task(void *arg)
{
    ...
    while (!lte_connect_task_close) {
        ...
        if (g_lte_config_queue != NULL) {
            if (xQueueReceive(g_lte_config_queue, &lte_cfg, pdMS_TO_TICKS(100)) == pdTRUE) {
                /* Validate APN length */
                ...
                if (apn_len > 0 && apn_len < sizeof(s_lte_apn)) {
                    ...
                    if (lte_cfg.username[0] != '\0') {
                        strncpy(s_lte_username, lte_cfg.username, sizeof(s_lte_username) - 1);
                        s_lte_username[sizeof(s_lte_username) - 1] = '\0';
                    }
                    
                    if (lte_cfg.password[0] != '\0') {
                        strncpy(s_lte_password, lte_cfg.password, sizeof(s_lte_password) - 1);
                    ...
                    esp_err_t ret = lte_init_with_config(s_lte_apn, s_lte_username, s_lte_password);
                    if (ret != ESP_OK) {
...
void lte_connect_task_start(void)
{
    ...
    esp_err_t ret = esp_event_handler_register(LTE_HANDLER_EVENT, 
                                               ESP_EVENT_ANY_ID,
                                               &lte_event_callback, 
                                               NULL);
    if (ret != ESP_OK) {
    ...
    BaseType_t task_ret = xTaskCreate(lte_config_task, 
                                     "lte_config_task", 
                                     4096, 
                                     NULL, 
                                     5, 
                                     &lte_config_task_handle);
    if (task_ret != pdPASS) {
...
void lte_connect_task_stop(void)
{
    ...
    esp_event_handler_unregister(LTE_HANDLER_EVENT, ESP_EVENT_ANY_ID, &lte_event_callback);
