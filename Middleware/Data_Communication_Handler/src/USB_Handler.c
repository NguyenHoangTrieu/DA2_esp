...
void jtag_task_start(void) {
  ...
  // Create jtag task
  task_created =
      xTaskCreatePinnedToCore(jtag_task, "usb_serial_jtag_echo", 4096, NULL,
                              JTAG_TASK_PRIORITY, &jtag_task_hdl, 0);
  assert(task_created == pdTRUE);
}

void jtag_task_stop(void) { close_jtag_task = true; }
  ...
  while (!close_usb_otg_rw_task) {
    ...
    if (dev != NULL) {
      if (dev->ep_out_addr == 0x00 || dev->ep_in_addr == 0x00) {
        ESP_LOGW("USB_OTG_RW",
                 "Device addr %d is not a valid serial device. Skipping.",
                 dev->dev_addr);
...
void usb_host_lib_task(void *arg) {
  ...
  ESP_LOGI(
      TAG,
      "No more clients and devices or close task, uninstall USB Host library");
...
void usb_host_lib_task_start(void) {
  ...
  task_created =
      xTaskCreatePinnedToCore(usb_host_lib_task, "usb_host", 4096, NULL,
                              HOST_LIB_TASK_PRIORITY, &usb_host_task_hdl, 0);
  assert(task_created == pdTRUE);
}
...
void class_driver_task_start(void) {
  ...
  task_created =
      xTaskCreatePinnedToCore(class_driver_task, "class", 4 * 1024, NULL,
