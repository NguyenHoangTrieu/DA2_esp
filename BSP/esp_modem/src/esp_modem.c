...
modem_dte_t *esp_modem_dte_init(const esp_modem_dte_config_t *config)
{
    ...
    if (config->flow_control == MODEM_FLOW_CONTROL_HW) {
        res = uart_set_hw_flow_ctrl(esp_dte->uart_port, UART_HW_FLOWCTRL_CTS_RTS, UART_HW_FIFO_LEN(esp_dte->uart_port) - 8);
    } else if (config->flow_control == MODEM_FLOW_CONTROL_SW) {
        res = uart_set_sw_flow_ctrl(esp_dte->uart_port, true, 8, UART_HW_FIFO_LEN(esp_dte->uart_port) - 8);
    }
...
{
    return esp_event_handler_register(ESP_MODEM_EVENT, event_id, handler, handler_args);
}
