...
esp_err_t bq25892_init(void) {
  esp_err_t ret =
      i2c_dev_support_add_device(BQ25892_I2C_ADDR, BQ25892_I2C_FREQ_HZ, &s_dev);
  if (ret != ESP_OK) {
  ...
  /* REG00: EN_ILIM=1 (use pin limit), IINLIM=18 → 1000mA
   *   IINLIM formula: I(mA) = 100 + IINLIM×50  →  1000 = 100 + 18×50  →
   * IINLIM=18 */
  ret = reg_write(BQ25892_REG00, 0x52); /* 0b0101_0010 */
  if (ret != ESP_OK)
    goto fail;

  /* REG04: ICHG=15 → 960mA (fast charge current limit)
   *   ICHG formula: I(mA) = ICHG×64  →  960 = 15×64  → ICHG=15 */
  ret = reg_write(BQ25892_REG04, 0x0F); /* 0b0000_1111 */
  if (ret != ESP_OK)
    goto fail;
  ...
  ESP_LOGI(TAG, "Initialized: IINLIM=1000mA, ICHG=960mA, VREG=4096mV (4.1V), "
                "charging enabled");
  return ESP_OK;
...
esp_err_t bq25892_set_charge_enable(bool enable) {
  ...
  esp_err_t ret = reg_modify(BQ25892_REG03, BQ25892_CHG_CONFIG_BIT,
...
esp_err_t bq25892_set_otg(bool enable) {
  if (!s_dev)
    return ESP_ERR_INVALID_STATE;
  return reg_modify(BQ25892_REG03, BQ25892_OTG_CONFIG_BIT,
...
esp_err_t bq25892_set_batfet_disable(bool disable) {
  if (!s_dev)
    return ESP_ERR_INVALID_STATE;
  esp_err_t ret = reg_modify(BQ25892_REG09, BQ25892_BATFET_DIS_BIT,
...
esp_err_t bq25892_read_status(bq25892_status_t *status) {
  ...
  switch (chrg) {
  case BQ25892_CHRG_STAT_PRE_CHG:
    status->chrg_status = BQ25892_STATUS_PRE_CHARGING;
    break;
  case BQ25892_CHRG_STAT_FAST_CHG:
    status->chrg_status = BQ25892_STATUS_FAST_CHARGING;
    break;
  case BQ25892_CHRG_STAT_DONE:
    status->chrg_status = BQ25892_STATUS_CHARGE_DONE;
    break;
  default:
    status->chrg_status = BQ25892_STATUS_NOT_CHARGING;
    break;
  }
