/*
 * ESP Modem UART DTE implementation
 */
#include "esp_log.h"
...
#define ESP_MODEM_UART_LINE_BUFFER_SIZE (CONFIG_UART_RX_BUFFER_SIZE / 2)
#define ESP_MODEM_UART_EVENT_QUEUE_SIZE (16)
 ...
 *
 */
static const char *MODEM_TAG = "esp-modem-uart";
#define MODEM_CHECK(a, str, goto_tag, ...)                                     \
  do {                                                                         \
    if (!(a)) {                                                                \
      ESP_LOGE(MODEM_TAG, "%s(%d): " str, __FUNCTION__, __LINE__,              \
               ##__VA_ARGS__);                                                 \
      goto goto_tag;                                                           \
    }                                                                          \
  } while (0)

ESP_EVENT_DEFINE_BASE(ESP_MODEM_UART_EVENT);

/**
 * @brief ESP32 Modem UART DTE
 *
 */
typedef struct {
  ...
  SemaphoreHandle_t
      process_sem;    /*!< Semaphore used for indicating processing status */
  modem_dte_t parent; /*!< DTE interface that should extend */
  esp_modem_uart_on_receive receive_cb; /*!< ptr to data reception */
...
uint8_t crc8(const char *src, size_t len, uint8_t polynomial,
...
esp_err_t esp_modem_uart_set_rx_cb(modem_dte_t *dte,
                                   esp_modem_uart_on_receive receive_cb,
                                   void *receive_cb_ctx) {
  esp_modem_uart_dte_t *esp_dte =
      __containerof(dte, esp_modem_uart_dte_t, parent);
 ...
 *
 * @param esp_dte ESP modem UART DTE object
 * @return esp_err_t
 *      - ESP_OK on success
 *      - ESP_FAIL on error
 */
static esp_err_t esp_dte_handle_line(esp_modem_uart_dte_t *esp_dte) {
  ...
  if (len > 2 && !is_only_cr_lf(line, len)) {
    ...
    MODEM_CHECK(dce->handle_line(dce, line) == ESP_OK, "handle line failed",
                err_handle);
  }
  return ESP_OK;
err_handle:
  /* Send ESP_MODEM_UART_EVENT_UNKNOWN signal to event loop */
  esp_event_post_to(esp_dte->event_loop_hdl, ESP_MODEM_UART_EVENT,
                    ESP_MODEM_UART_EVENT_UNKNOWN, (void *)line,
                    strlen(line) + 1, pdMS_TO_TICKS(100));
err:
  return ESP_FAIL;
}
 ...
 *      - ESP_FAIL on error
 */
static esp_err_t esp_dte_handle_cmux_frame(esp_modem_uart_dte_t *esp_dte) {
  modem_dce_t *dce = esp_dte->parent.dce;
  ...
  ESP_LOGD(MODEM_TAG, "CMUX FR: A:%02x T:%02x L:%d Buf:%d", dlci, type, length,
           esp_dte->buffer_len);
  //    printf("buffer >>> ");
  ...
  if (dce->handle_cmux_frame != NULL) {
    MODEM_CHECK(dce->handle_cmux_frame(dce, frame) == ESP_OK,
                "handle cmux frame failed", err_handle);
  } else if ((type == FT_UIH || type == (FT_UIH | PF)) && dlci == 1 &&
             dce->handle_line != NULL && strlen(&frame[6]) > 2) {
    // Handle CONNECT message on DLCI 1
    ESP_LOGI(MODEM_TAG, "Handle Line: %s for DLCI 1", &frame[6]);
    MODEM_CHECK(dce->handle_line, "no handler for line", err_handle);
    MODEM_CHECK(dce->handle_line(dce, &frame[6]) == ESP_OK,
                "handle line failed", err_handle);
    dce->handle_line = NULL;
  } else if ((type == FT_UIH || type == (FT_UIH | PF)) && dlci == 2 &&
             dce->handle_line != NULL) {
  ...
  } else if ((type == FT_UIH || type == (FT_UIH | PF)) && length && dlci == 1 &&
             esp_dte->receive_cb != NULL) {
    // Handle DCLI 1
    ESP_LOGD(MODEM_TAG, "Pass data with length %d from DLCI: %d to receive_cb",
             length, dlci);
...
err_handle:
  /* Send ESP_MODEM_UART_EVENT_UNKNOWN signal to event loop */
  esp_event_post_to(esp_dte->event_loop_hdl, ESP_MODEM_UART_EVENT,
                    ESP_MODEM_UART_EVENT_UNKNOWN, "cmux frame invalid", 4,
                    pdMS_TO_TICKS(100));
 ...
 *
 * @param esp_dte ESP32 Modem UART DTE object
 */
static void esp_handle_uart_pattern(esp_modem_uart_dte_t *esp_dte) {
  ...
  if (pos != -1) {
    if (pos < esp_dte->line_buffer_size - 1) {
      ...
      ESP_LOGW(MODEM_TAG, "ESP Modem UART Line buffer too small");
      read_len = esp_dte->line_buffer_size - 1;
    }
    read_len = uart_read_bytes(esp_dte->uart_port, esp_dte->buffer, read_len,
                               pdMS_TO_TICKS(100));
    if (read_len) {
...
static void esp_handle_uart_frame(esp_modem_uart_dte_t *esp_dte) {
...
handle:
  ...
  ESP_LOGD(MODEM_TAG, "Check frame with buffer length: %d, frame length: %d",
           esp_dte->buffer_len, frame_length_full);
  ...
  if (esp_dte->buffer_len > frame_length_full) {
    ...
    ESP_LOGD(MODEM_TAG, "Copy %d from next frame to beginning of the buffer",
             frame_length_next);
    //        printf("copy >>> ");
    ...
    memcpy(esp_dte->buffer, &esp_dte->buffer[frame_length_full],
           frame_length_next);
 ...
 * @param esp_dte ESP32 Modem UART DTE object
 */
static void esp_handle_uart_data(esp_modem_uart_dte_t *esp_dte) {
  size_t length = 0;
  length = MIN(esp_dte->line_buffer_size, length);
  uart_get_buffered_data_len(esp_dte->uart_port, &length);
  length =
      uart_read_bytes(esp_dte->uart_port, &esp_dte->buffer[esp_dte->buffer_len],
  ...
  if (esp_dte->buffer_len > 0) {
    if (esp_dte->buffer[0] == SOF_MARKER) {
      ...
      //            ESP_LOGW(MODEM_TAG, "RX data is out of sync. Missing SOF.
      //            Buffer length: %d", esp_dte->buffer_len);
      //            memcpy(esp_dte->buffer, &esp_dte->buffer[1],
      //            esp_dte->buffer_len - 1); esp_dte->buffer_len--;
...
static void uart_event_task_entry(void *param) {
  esp_modem_uart_dte_t *esp_dte = (esp_modem_uart_dte_t *)param;
  uart_event_t event;
  while (1) {
 ...
 *      - ESP_FAIL on error
 */
static esp_err_t esp_modem_uart_dte_send_cmd(modem_dte_t *dte,
                                             const char *command,
                                             uint32_t timeout) {
  ...
  /* Check timeout */
  MODEM_CHECK(xSemaphoreTake(esp_dte->process_sem, pdMS_TO_TICKS(timeout)) ==
                  pdTRUE,
              "process command timeout", err);
  ret = ESP_OK;
err:
  dce->handle_line = NULL;
  return ret;
...
static esp_err_t esp_modem_uart_dte_send_sabm(modem_dte_t *dte, uint8_t dlci,
                                              uint32_t timeout) {
  esp_err_t ret = ESP_FAIL;
  modem_dce_t *dce = dte->dce;
  MODEM_CHECK(dce, "DTE has not yet bind with DCE", err);
 ...
 *      - ESP_FAIL on error
 */
static esp_err_t esp_modem_uart_dte_send_cmux_cmd(modem_dte_t *dte,
                                                  const char *command,
                                                  uint32_t timeout) {
  esp_err_t ret = ESP_FAIL;
  modem_dce_t *dce = dte->dce;
  ...
  frame[4 + strlen(command)] =
      0xFF - crc8(&frame[1], 3, FCS_POLYNOMIAL, FCS_INIT_VALUE, true);
 ...
 */
static int esp_modem_uart_dte_send_data(modem_dte_t *dte, const char *data,
                                        uint32_t length) {
 ...
 * @return int actual length of data that has been send out
 */
static int esp_modem_uart_dte_send_cmux_data(modem_dte_t *dte, const char *data,
                                             uint32_t length) {
  MODEM_CHECK(data, "data is NULL", err);
  esp_modem_uart_dte_t *esp_dte =
      __containerof(dte, esp_modem_uart_dte_t, parent);
 ...
 *      ESP_FAIL on error
 */
static esp_err_t esp_modem_uart_dte_send_wait(modem_dte_t *dte,
  ...
  esp_modem_uart_dte_t *esp_dte =
      __containerof(dte, esp_modem_uart_dte_t, parent);
  // We'd better disable pattern detection here for a moment in case prompt
  // string contains the pattern character
  uart_disable_pattern_det_intr(esp_dte->uart_port);
  // uart_disable_rx_intr(esp_dte->uart_port);
  MODEM_CHECK(uart_write_bytes(esp_dte->uart_port, data, length) >= 0,
              "uart write bytes failed", err_write);
  uint32_t len = strlen(prompt);
  uint8_t *buffer = calloc(len + 1, sizeof(uint8_t));
  int res =
      uart_read_bytes(esp_dte->uart_port, buffer, len, pdMS_TO_TICKS(timeout));
  MODEM_CHECK(res >= len, "wait prompt [%s] timeout", err, prompt);
  MODEM_CHECK(!strncmp(prompt, (const char *)buffer, len),
              "get wrong prompt: %s", err, buffer);
  free(buffer);
  uart_enable_pattern_det_baud_intr(esp_dte->uart_port, '\n', 1,
                                    MIN_PATTERN_INTERVAL, MIN_POST_IDLE,
                                    MIN_PRE_IDLE);
  return ESP_OK;
err:
  free(buffer);
 ...
 *      - ESP_FAIL on error
 */
static esp_err_t esp_modem_uart_dte_change_mode(modem_dte_t *dte,
                                                modem_mode_t new_mode) {
  modem_dce_t *dce = dte->dce;
  MODEM_CHECK(dce, "DTE has not yet bind with DCE", err);
  esp_modem_uart_dte_t *esp_dte =
      __containerof(dte, esp_modem_uart_dte_t, parent);
  ...
  case MODEM_PPP_MODE:
    ...
    MODEM_CHECK(dce->set_working_mode(dce, new_mode) == ESP_OK,
                "set new working mode:%d failed", err, new_mode);
    uart_disable_pattern_det_intr(esp_dte->uart_port);
  ...
  case MODEM_COMMAND_MODE:
    ...
    uart_enable_pattern_det_baud_intr(esp_dte->uart_port, '\n', 1,
                                      MIN_PATTERN_INTERVAL, MIN_POST_IDLE,
                                      MIN_PRE_IDLE);
    //        uart_pattern_queue_reset(esp_dte->uart_port,
...
static esp_err_t esp_modem_uart_dte_process_cmd_done(modem_dte_t *dte) {
  esp_modem_uart_dte_t *esp_dte =
      __containerof(dte, esp_modem_uart_dte_t, parent);
 ...
 *      - ESP_FAIL on error
 */
static esp_err_t esp_modem_uart_dte_deinit(modem_dte_t *dte) {
  esp_modem_uart_dte_t *esp_dte =
      __containerof(dte, esp_modem_uart_dte_t, parent);
...
modem_dte_t *
esp_modem_uart_dte_init(const esp_modem_uart_dte_config_t *config) {
  esp_err_t res;
  /* malloc memory for esp_dte object */
  esp_modem_uart_dte_t *esp_dte = calloc(1, sizeof(esp_modem_uart_dte_t));
  ...
  /* Bind methods */
  esp_dte->parent.send_cmd = esp_modem_uart_dte_send_cmd;
  esp_dte->parent.send_cmux_cmd = esp_modem_uart_dte_send_cmux_cmd;
  esp_dte->parent.send_sabm = esp_modem_uart_dte_send_sabm;
  esp_dte->parent.send_data = esp_modem_uart_dte_send_data;
  esp_dte->parent.send_cmux_data = esp_modem_uart_dte_send_cmux_data;
  esp_dte->parent.send_wait = esp_modem_uart_dte_send_wait;
  esp_dte->parent.change_mode = esp_modem_uart_dte_change_mode;
  esp_dte->parent.process_cmd_done = esp_modem_uart_dte_process_cmd_done;
  esp_dte->parent.deinit = esp_modem_uart_dte_deinit;
  ...
  uart_config_t uart_config = {
      ...
      .flow_ctrl = (config->flow_control == MODEM_FLOW_CONTROL_HW)
                       ? UART_HW_FLOWCTRL_CTS_RTS
                       : UART_HW_FLOWCTRL_DISABLE};
  MODEM_CHECK(uart_param_config(esp_dte->uart_port, &uart_config) == ESP_OK,
              "config uart parameter failed", err_uart_config);
  ...
  if (config->flow_control == MODEM_FLOW_CONTROL_HW) {
    res = uart_set_hw_flow_ctrl(esp_dte->uart_port, UART_HW_FLOWCTRL_CTS_RTS,
                                UART_HW_FIFO_LEN(esp_dte->uart_port) - 8);
  } else if (config->flow_control == MODEM_FLOW_CONTROL_SW) {
    res = uart_set_sw_flow_ctrl(esp_dte->uart_port, true, 8,
                                UART_HW_FIFO_LEN(esp_dte->uart_port) - 8);
  }
  MODEM_CHECK(res == ESP_OK, "config uart flow control failed",
              err_uart_config);
  /* Install UART driver and get event queue used inside driver */
  res = uart_driver_install(esp_dte->uart_port, config->rx_buffer_size,
                            config->tx_buffer_size, config->event_queue_size,
                            &(esp_dte->event_queue), 0);
  ...
  res |=
      uart_pattern_queue_reset(esp_dte->uart_port, config->pattern_queue_size);
  ...
  //     .queue_size = ESP_MODEM_UART_EVENT_QUEUE_SIZE,
  //     .task_name = NULL
  // };
  // MODEM_CHECK(esp_event_loop_create(&loop_args, &esp_dte->event_loop_hdl) ==
  // ESP_OK, "create event loop failed", err_eloop);
  /* Create semaphore */
  esp_dte->process_sem = xSemaphoreCreateBinary();
  MODEM_CHECK(esp_dte->process_sem, "create process semaphore failed", err_sem);
  /* Create UART Event task */
  BaseType_t ret =
      xTaskCreate(uart_event_task_entry,          // Task Entry
...
esp_err_t esp_modem_uart_set_event_handler(esp_event_handler_t handler,
                                           int32_t event_id,
                                           void *handler_args) {
  return esp_event_handler_register(ESP_MODEM_UART_EVENT, event_id, handler,
                                    handler_args);
}

esp_err_t esp_modem_uart_remove_event_handler(esp_event_handler_t handler) {
  return esp_event_handler_unregister(ESP_MODEM_UART_EVENT, ESP_EVENT_ANY_ID,
                                      handler);
}
...
esp_err_t esp_modem_uart_start_ppp(modem_dte_t *dte) {
  ...
  MODEM_CHECK(dce->define_pdp_context(dce, 1, "IP",
                                      CONFIG_COMPONENT_MODEM_APN) == ESP_OK,
              "set MODEM APN failed", err);
  /* Enter PPP mode */
  MODEM_CHECK(dte->change_mode(dte, MODEM_PPP_MODE) == ESP_OK,
              "enter ppp mode failed", err);

  /* post PPP mode started event */
  esp_event_post_to(esp_dte->event_loop_hdl, ESP_MODEM_UART_EVENT,
                    ESP_MODEM_UART_EVENT_PPP_START, NULL, 0, 0);
  return ESP_OK;
err:
  return ESP_FAIL;
}
...
esp_err_t esp_modem_uart_start_cmux(modem_dte_t *dte) {
  ...
  /* Enter cmux mode */
  MODEM_CHECK(dte->change_mode(dte, MODEM_CMUX_MODE) == ESP_OK,
              "enter command mode failed", err);
...
esp_err_t esp_modem_uart_stop_ppp(modem_dte_t *dte) {
  ...
  esp_event_post_to(esp_dte->event_loop_hdl, ESP_MODEM_UART_EVENT,
                    ESP_MODEM_UART_EVENT_PPP_STOP, NULL, 0, 0);
  /* Enter command mode */
  MODEM_CHECK(dte->change_mode(dte, MODEM_COMMAND_MODE) == ESP_OK,
              "enter command mode failed", err);
  /* Hang up */
