...
static esp_modem_usb_dce_t *s_dce = NULL;
...
typedef struct {
    ...
    esp_modem_usb_recov_gpio_t *power_pin;
    esp_modem_usb_recov_gpio_t *reset_pin;
    esp_err_t (*reset)(esp_modem_usb_dce_t *dce);
    esp_err_t (*power_up)(esp_modem_usb_dce_t *dce);
    esp_err_t (*power_down)(esp_modem_usb_dce_t *dce);
    esp_modem_usb_recov_resend_t *re_sync;
    esp_modem_usb_recov_resend_t *re_store_profile;
} modem_board_t;
...
static esp_err_t modem_board_handle_powerup(esp_modem_usb_dce_t *dce, const char *line)
{
...
static esp_err_t modem_board_reset(esp_modem_usb_dce_t *dce)
{
...
static esp_err_t modem_board_power_up(esp_modem_usb_dce_t *dce)
{
    modem_board_t *board = __containerof(dce, modem_board_t, parent);
...
static esp_err_t modem_board_power_down(esp_modem_usb_dce_t *dce)
{
    modem_board_t *board = __containerof(dce, modem_board_t, parent);
...
static esp_err_t my_recov(esp_modem_usb_recov_resend_t *retry_cmd, esp_err_t err, int timeouts, int errors)
{
    esp_modem_usb_dce_t *dce = retry_cmd->dce;
    ESP_LOGI(TAG, "Current timeouts: %d and errors: %d", timeouts, errors);
    if (err != ESP_OK) {
    if (timeouts + errors < 2) {
        ESP_LOGW(TAG, "Try to exit ppp mode........");
        dce->mode = ESP_MODEM_USB_TRANSITION_MODE;
...
static esp_err_t modem_board_start_up(esp_modem_usb_dce_t *dce)
{
...
static esp_err_t modem_board_dce_deinit(esp_modem_usb_dce_t *dce)
{
    ...
    esp_err_t err = esp_modem_usb_command_list_deinit(&board->parent);
    if (err == ESP_OK) {
    free(dce);
    }
    s_dce = NULL;
...
static esp_modem_usb_dce_t *modem_board_create(esp_modem_usb_dce_config_t *config)
{
    ...
    board->re_sync = esp_modem_usb_recov_resend_new(&board->parent, board->parent.sync, my_recov, 5, 5);
               ...
               int32_t event_id, void *event_data)
{
    ...
    } else if (event_base == ESP_MODEM_USB_EVENT) {
    switch (event_id) {
    default:
...
static bool _ppp_network_start(esp_modem_usb_dte_t *dte)
{
    dte->dce->mode = ESP_MODEM_USB_TRANSITION_MODE;
    if (esp_modem_usb_start_ppp(dte) == ESP_OK) {
    return true;
    }
    return false;
}

static bool _ppp_network_stop(esp_modem_usb_dte_t *dte)
{
    if (esp_modem_usb_stop_ppp(dte) == ESP_OK) {
    return true;
    }
    return false;
}
...
static void _modem_daemon_task(void *param)
{
    ...
    // init the USB DTE
    esp_modem_usb_dte_config_t dte_config = ESP_MODEM_USB_DTE_DEFAULT_CONFIG();
    ...
    esp_modem_usb_dce_config_t dce_config = ESP_MODEM_USB_DCE_DEFAULT_CONFIG(CONFIG_MODEM_PPP_APN);
    esp_netif_config_t ppp_netif_config = ESP_NETIF_DEFAULT_PPP();

    // Initialize esp-modem units, DTE, DCE, ppp-netif
    esp_modem_usb_dte_t *dte = esp_modem_usb_dte_new(&dte_config);
    assert(dte != NULL);
    esp_modem_usb_dce_t *dce = modem_board_create(&dce_config);
    assert(dce != NULL);
    ...
    s_dce = dce;
    ESP_ERROR_CHECK(esp_modem_usb_default_attach(dte, dce, ppp_netif));
    ESP_ERROR_CHECK(esp_modem_usb_set_event_handler(dte, on_modem_event, ESP_EVENT_ANY_ID, NULL));
    ...
    case STAGE_WAITING:
        ...
        break;
    case STAGE_SYNC:
        if (esp_modem_usb_default_start(dte) != ESP_OK) {
        retry_after_ms = 3000;
        ++stage_retry_times;
        } else {
        modem_stage = STAGE_WAITING;
...
_stage_succeed:
    ...
    // destroy dte & dce
    ESP_ERROR_CHECK(esp_modem_usb_default_destroy(dte));
...
esp_err_t modem_board_get_signal_quality(int *rssi, int *ber)
{
    ...
    esp_modem_usb_dce_csq_ctx_t result;
    esp_err_t err = esp_modem_usb_dce_get_signal_quality(s_dce, NULL, &result);
    if (err == ESP_OK) {
    if (rssi) {
        *rssi = result.rssi;
    }
...
esp_err_t modem_board_get_sim_cart_state(int *if_ready)
{
    ...
    MODEM_CHECK(s_dce != NULL, "modem not ready", ESP_ERR_INVALID_STATE);
    *if_ready = false;
    esp_modem_usb_dce_read_pin(s_dce, NULL, if_ready);
    if ((*if_ready) == false) {
    esp_modem_usb_dce_set_pin(s_dce, MODEM_SIM_PIN_PWD, NULL);
    } else {
    return ESP_OK;
    }

    return esp_modem_usb_dce_read_pin(s_dce, NULL, &if_ready);
}
...
esp_err_t modem_board_get_operator_state(char *buf, size_t buf_size)
{
    ...
    MODEM_CHECK(s_dce != NULL, "modem not ready", ESP_ERR_INVALID_STATE);
    return esp_modem_usb_dce_get_operator_name(s_dce, (void *)buf_size, (void *)buf);
}

esp_err_t modem_board_set_apn(const char *new_apn, bool force_enable)
{
    MODEM_CHECK(s_dce != NULL, "modem not ready", ESP_ERR_INVALID_STATE);
    if (esp_modem_usb_dce_set_apn(s_dce, new_apn) != ESP_OK) {
