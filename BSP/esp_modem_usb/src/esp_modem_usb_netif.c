 ...
 */
struct esp_modem_usb_netif_driver_s {
    esp_netif_driver_base_t base;           /*!< base structure reserved as esp-netif driver */
    esp_modem_usb_dte_t        *dte;            /*!< ptr to the esp_modem_usb objects (DTE) */
};

const char *esp_modem_usb_netif_event_to_name(int code)
{
    size_t i;
...
{
    esp_modem_usb_dte_t *dte = arg;
    if (event_id < NETIF_PP_PHASE_OFFSET) {
        ESP_LOGI(TAG, "PPP state changed event %"PRIi32": (%s)", event_id, esp_modem_usb_netif_event_to_name(event_id));
        // only notify the modem on state/error events, ignoring phase transitions
        esp_modem_usb_notify_ppp_netif_closed(dte);
    }
}
 ...
 *
 * @param h Opaque pointer representing esp-netif driver, esp_dte in this case of esp_modem_usb
 * @param data data buffer
 * @param length length of data to send
 *
 * @return ESP_OK on success
 */
static esp_err_t esp_modem_usb_dte_transmit(void *h, void *buffer, size_t len)
{
    esp_modem_usb_dte_t *dte = h;
 ...
 */
static esp_err_t esp_modem_usb_post_attach_init(esp_netif_t * esp_netif, void * args)
{
    esp_modem_usb_netif_driver_t *driver = args;
    esp_modem_usb_dte_t *dte = driver->dte;
    const esp_netif_driver_ifconfig_t driver_ifconfig = {
        .driver_free_rx_buffer = NULL,
        .transmit = esp_modem_usb_dte_transmit,
        .handle = dte
    };
...
{
    esp_modem_usb_netif_driver_t *driver = context;
    esp_netif_receive(driver->base.netif, buffer, len, NULL);
    return ESP_OK;
}

esp_modem_usb_netif_driver_t *esp_modem_usb_netif_new(esp_modem_usb_dte_t *dte)
{
    esp_modem_usb_netif_driver_t *driver = esp_modem_usb_netif_setup(dte);
    return driver;
}

esp_modem_usb_netif_driver_t *esp_modem_usb_netif_setup(esp_modem_usb_dte_t *dte)
{
    esp_modem_usb_netif_driver_t *driver =  calloc(1, sizeof(esp_modem_usb_netif_driver_t));
    if (driver == NULL) {
        ESP_LOGE(TAG, "Cannot allocate esp_modem_usb_netif_driver_t");
        goto drv_create_failed;
    }
    esp_err_t err = esp_modem_usb_set_rx_cb(dte, modem_netif_receive_cb, driver);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "esp_modem_usb_set_rx_cb failed with: %d", err);
        goto drv_create_failed;
    }

    driver->base.post_attach = esp_modem_usb_post_attach_init;
    driver->dte = dte;
    return driver;
...
void esp_modem_usb_netif_destroy(esp_modem_usb_netif_driver_t *driver)
{
    esp_netif_destroy(driver->base.netif);
    return esp_modem_usb_netif_teardown(driver);
}

void esp_modem_usb_netif_teardown(esp_modem_usb_netif_driver_t *driver)
{
    free(driver);
}

esp_err_t esp_modem_usb_netif_clear_default_handlers(esp_modem_usb_netif_driver_t *h)
{
    esp_modem_usb_netif_driver_t *driver = h;
    esp_err_t ret;
    ret = esp_modem_usb_remove_event_handler(driver->dte, esp_netif_action_start, ESP_MODEM_USB_EVENT_PPP_START);
    if (ret != ESP_OK) {
        goto clear_event_failed;
    }
    ret = esp_modem_usb_remove_event_handler(driver->dte, esp_netif_action_stop, ESP_MODEM_USB_EVENT_PPP_STOP);
    if (ret != ESP_OK) {
        goto clear_event_failed;
    }
...
esp_err_t esp_modem_usb_netif_set_default_handlers(esp_modem_usb_netif_driver_t *h, esp_netif_t * esp_netif)
{
    esp_modem_usb_netif_driver_t *driver = h;
    esp_err_t ret;
    ret = esp_modem_usb_set_event_handler(driver->dte, esp_netif_action_start, ESP_MODEM_USB_EVENT_PPP_START, esp_netif);
    if (ret != ESP_OK) {
        goto set_event_failed;
    }
    ret = esp_modem_usb_set_event_handler(driver->dte, esp_netif_action_stop, ESP_MODEM_USB_EVENT_PPP_STOP, esp_netif);
    if (ret != ESP_OK) {
        goto set_event_failed;
    }
...
set_event_failed:
    ...
    esp_modem_usb_netif_clear_default_handlers(driver);
    return ESP_FAIL;
}