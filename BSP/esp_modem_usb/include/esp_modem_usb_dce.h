#ifndef ESP_MODEM_USB_DCE_H_
#define ESP_MODEM_USB_DCE_H_
 ...
 */
typedef enum {
    ESP_MODEM_USB_STATE_PROCESSING, /*!< In processing */
    ESP_MODEM_USB_STATE_SUCCESS,    /*!< Process successfully */
    ESP_MODEM_USB_STATE_FAIL        /*!< Process failed */
} esp_modem_usb_state_t;

/**
 * @brief Generic command type used in DCE unit
 */
typedef esp_err_t (*dce_command_t)(esp_modem_usb_dce_t *dce, void *param, void *result);

/**
 * @brief Type of line handlers called from DTE upon line response reception
 */
typedef esp_err_t (*esp_modem_usb_dce_handle_line_t)(esp_modem_usb_dce_t *dce, const char *line);

/**
 * @brief DCE(Data Communication Equipment)
 *
 */
struct esp_modem_usb_dce {
    esp_modem_usb_state_t state;                                                    /*!< Modem working state */
    esp_modem_usb_mode_t mode;                                                      /*!< Working mode */
    esp_modem_usb_dte_t *dte;                                                       /*!< DTE which connect to DCE */
    struct esp_modem_usb_dce_cmd_list *dce_cmd_list;
    esp_modem_usb_dce_config_t config;
    esp_modem_usb_dce_handle_line_t handle_line;                                    /*!< Handle line strategy */

    void *handle_line_ctx;                                                      /*!< DCE context reserved for handle_line
                                                                                    processing */
    // higher level actions DCE unit can take
    esp_err_t (*set_working_mode)(esp_modem_usb_dce_t *dce, esp_modem_usb_mode_t mode); /*!< Set working mode */
    esp_err_t (*deinit)(esp_modem_usb_dce_t *dce);                                  /*!< Destroys the DCE */
    esp_err_t (*start_up)(esp_modem_usb_dce_t *dce);                                /*!< Start-up sequence */
