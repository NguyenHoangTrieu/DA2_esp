#ifndef _ESP_MODEM_USB_H_
#define _ESP_MODEM_USB_H_
 ...
 *
 */
typedef struct esp_modem_usb_dte esp_modem_usb_dte_t;
typedef struct esp_modem_usb_dce esp_modem_usb_dce_t;

/**
 * @brief Declare Event Base for ESP Modem
 *
 */
ESP_EVENT_DECLARE_BASE(ESP_MODEM_USB_EVENT);
 ...
 */
typedef enum {
    ESP_MODEM_USB_EVENT_PPP_START = 0,       /*!< ESP Modem Start PPP Session */
    ESP_MODEM_USB_EVENT_PPP_RESTART = 1,     /*!< ESP Modem Restart PPP Session */
    ESP_MODEM_USB_EVENT_PPP_STOP  = 3,       /*!< ESP Modem Stop PPP Session*/
    ESP_MODEM_USB_EVENT_UNKNOWN   = 4        /*!< ESP Modem Unknown Response */
} esp_modem_usb_event_t;
/**
 * @defgroup ESP_MODEM_USB_DTE_TYPES DTE Types
 * @brief  Configuration and related types used to init and setup a new DTE object
 */

/** @addtogroup ESP_MODEM_USB_DTE_TYPES
 * @{
 */
 ...
 */
typedef enum {
    ESP_MODEM_USB_FLOW_CONTROL_NONE = 0,
    ESP_MODEM_USB_FLOW_CONTROL_SW,
    ESP_MODEM_USB_FLOW_CONTROL_HW
} esp_modem_usb_flow_ctrl_t;
...
typedef struct {
    ...
    esp_modem_usb_flow_ctrl_t flow_control; /*!< Flow control type */
...
} esp_modem_usb_dte_config_t;

/**
 * @brief ESP Modem DTE Default Configuration
 *
 */
#define ESP_MODEM_USB_DTE_DEFAULT_CONFIG()          \
    {                                           \
        .port_num = UART_NUM_1,                 \
        ...
        .baud_rate = 115200,                    \
        .flow_control = ESP_MODEM_USB_FLOW_CONTROL_NONE,\
        .tx_io_num = 25,                        \
        .rx_io_num = 26,                        \
        .rts_io_num = 27,                       \
        .cts_io_num = 23,                       \
...
/**
 * @defgroup ESP_MODEM_USB_DCE_TYPES DCE Types
 * @brief  Configuration and related types used to init and setup a new DCE object
 */

/** @addtogroup ESP_MODEM_USB_DCE_TYPES
 * @{
 */

/**
 * @brief PDP context type used as an input parameter to esp_modem_usb_dce_set_pdp_context
 * also used as a part of configuration structure
 */
typedef struct esp_modem_usb_dce_pdp_ctx_s {
...
} esp_modem_usb_dce_pdp_ctx_t;

/**
 * @brief Devices that the DCE will act as
 *
 */
typedef enum esp_modem_usb_dce_device_e {
    ESP_MODEM_USB_DEVICE_UNSPECIFIED,
} esp_modem_usb_dce_device_t;

/**
 * @brief DCE's configuration structure
 */
typedef struct esp_modem_usb_dce_config_s {
    esp_modem_usb_dce_pdp_ctx_t pdp_context;    /*!<  modem PDP context including APN */
    ...
    esp_modem_usb_dce_device_t device;          /*!<  predefined device enum that the DCE will initialise as */
} esp_modem_usb_dce_config_t;

/**
* @brief Default configuration of DCE unit of ESP-MODEM
*
*/
#define ESP_MODEM_USB_DCE_DEFAULT_CONFIG(APN) \
    {                                  \
        .pdp_context = {               \
            .cid = 1,                  \
            .type = "IP",              \
        ...
        .device = ESP_MODEM_USB_DEVICE_UNSPECIFIED   \
    }

/**
 * @defgroup ESP_MODEM_USB_DTE_DCE DCE and DCE object init and setup API
 * @brief  Creating and init objects of DTE and DCE
 */

/** @addtogroup ESP_MODEM_USB_DTE_DCE
 * @{
 */
 ...
 * "sub-class" of esp_modem_usb_dce_t
 *
...
/**
 * @defgroup ESP_MODEM_USB_EVENTS Event handling API
 */

/** @addtogroup ESP_MODEM_USB_EVENTS
 * @{
 */
...
/**
 * @defgroup ESP_MODEM_USB_LIFECYCLE Modem lifecycle API
 * @brief  Basic modem API to start/stop the PPP mode
 */

/** @addtogroup ESP_MODEM_USB_LIFECYCLE
 * @{
 */
 ...
 * esp_modem_usb_default_attach(). Then it deletes the DTE itself.
 *
 * @param dte Modem DTE Object
 * @return esp_err_t
 *      - ESP_OK on success
